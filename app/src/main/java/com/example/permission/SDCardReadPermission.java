...
public class SDCardReadPermission {

    private static boolean result = false;

    public static boolean applyForSDCardReadPermission(final Activity activity) {
        if (Build.VERSION.SDK_INT <= 23) {
            Permissions4M.get(activity)
                    .requestForce(true)// 是否强制弹出权限申请对话框
                    .requestUnderM(true)// 是否支持 5.0 权限申请
                    .requestPermissions(Manifest.permission.READ_EXTERNAL_STORAGE)// 申请的权限
                    .requestCodes(16)// 申请权限的权限码
                    ...
                    .requestCustomRationaleListener(new ListenerWrapper.PermissionCustomRationaleListener() {
                        ...
                        public void permissionCustomRationale(int i) {
                            new AlertDialog.Builder(activity)
                                    .setCancelable(false)
                                    .setTitle("授权")
                                    .setMessage("APP需要将图片存储到SD卡中，是否授予SD卡的使用权限?\n（不授予该权限则无法启用拍照功能）")
            ...
            permissions.requestEach(Manifest.permission.READ_EXTERNAL_STORAGE).subscribe(new Consumer<Permission>() {
                @Override
