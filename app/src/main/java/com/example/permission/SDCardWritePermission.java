...
public class SDCardWritePermission {

    private static boolean result = false;

    public static boolean applyForSDCardWritePermission(final Activity activity) {
        if (Build.VERSION.SDK_INT <= 23) {
            Permissions4M.get(activity)
                    .requestForce(true)// 是否强制弹出权限申请对话框
                    .requestUnderM(true)// 是否支持 5.0 权限申请
                    .requestPermissions(Manifest.permission.WRITE_EXTERNAL_STORAGE)// 申请的权限
                    .requestCodes(17)// 申请权限的权限码
                    ...
                    .requestCustomRationaleListener(new ListenerWrapper.PermissionCustomRationaleListener() {
                        ...
                        public void permissionCustomRationale(int i) {
                            new AlertDialog.Builder(activity)
                                    .setCancelable(false)
                                    .setTitle("授权")
                                    .setMessage("APP需要读取SD卡中的图片，是否授予SD卡的使用权限?\n（不授予该权限则无法启用拍照功能）")
                    ...
                    .requestPage(new ListenerWrapper.PermissionPageListener() {
                        ...
                        public void pageIntent(int i, final Intent intent) {
                            new AlertDialog.Builder(activity)
                                    .setCancelable(false)
                                    .setTitle("授权")
                                    .setMessage("APP需要读取SD卡中的图片，需要授予SD卡的使用权限?\n（不授予该权限则无法启用拍照功能）")
            ...
            permissions.requestEach(Manifest.permission.WRITE_EXTERNAL_STORAGE).subscribe(new Consumer<Permission>() {
                @Override
